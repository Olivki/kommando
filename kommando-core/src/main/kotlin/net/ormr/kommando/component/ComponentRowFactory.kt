...
public class ComponentRowFactory(
    private val groupId: String,
    private val factories: List<ComponentFactory>,
    private val interactionIdProvider: ComponentInteractionIdProvider,
) {
    private fun ComponentFactory.create(): VisibleComponent<*> = when (this) {
        is ComponentFactory.Row -> error("Nested rows are not allowed")
    ...
    public fun create(): ComponentRow = ComponentRow(
        components = factories.map { it.create() },
    )
}